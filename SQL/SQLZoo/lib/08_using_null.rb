...
def all_teachers_join
  ...
  FROM teachers LEFT JOIN depts ON depts.id = dept_id
  SQL
end
...
def all_depts_join
  ...
  execute(<<-SQL)
  SELECT teachers.name, depts.name
  FROM teachers RIGHT JOIN depts ON depts.id = dept_id
  SQL
end
...
def dept_staff_counts
  ...
  FROM depts LEFT JOIN teachers on depts.id = dept_id
  GROUP BY depts.name
  SQL
end
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
  SELECT name, CASE WHEN dept_id in (1,2) THEN 'Sci' ELSE 'Art' END
  FROM teachers
  SQL
end
...
def teachers_and_divisions_two
  ...
  # 'None' otherwise.
  execute(<<-SQL)
  SELECT name, CASE WHEN dept_id in (1,2) THEN 'Sci' WHEN dept_id =3 THEN 'Art' ELSE 'None' END
  FROM teachers
  SQL
end
