...
def biggest_cast
  ...
  Movie.joins(:actors).group(:id).order('COUNT(actors.id) DESC').limit(3).select('movies.id, movies.title')
end
...
def directed_by_one_of(them)
  ...
  Movie.joins(:director).where(actors: {name:them }).select("movies.id, movies.title")
end
